...
const OccasionsPage = async ({ searchParams }: { searchParams: SearchParamOcassion }) => {
  ...
  const payload = await getOccasions(searchParams);
  if ("error" in payload) {
    return (
  ...
  return (
    <section className="w-full px-4 md:px-20 py-6">
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-300">
          {t("number-of-products", {
            count: items.reduce((a, c) => a + (c.productsCount ?? 0), 0),
          })}
        </p>
      </header>
      ...
      <div className="mt-8">
        <PaginationComponent metaData={payload.metadata} />
      </div>
    </section>
  );
};
